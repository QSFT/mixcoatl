...
def get_rc_nodes(token, count):
    ...
    channel_list_response = requests.get(
        API_URL.format(api=channel_list) + '?token=' + token)
...
if not token:
    ...
    for f in glob.glob(configpath + "/k.*.config"):
        os.remove(f)
    env.hosts = get_rc_nodes(token, 7)
...
def gen_configs():
    with hide('output', 'running', 'warnings'), settings(warn_only=True):
        ...
        try:
            json_file_contents = json.loads(output)

            for i in json_file_contents:
                if i['customerManagementKey'] is True and i['name'] == 'AWSCustomerKey':
                    the_file = configpath + '/' + \
                        version + '-customerkey.config'
